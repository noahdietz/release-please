...
function gitHubOptions(yargs: yargs.Argv): yargs.Argv {
  return yargs
    ...
    .middleware(_argv => {
      const argv = _argv as GitHubArgs & { 'local-write'?: boolean };
      if (argv.localWrite || argv['local-write']) {
        argv.local = true;
        argv.localWrite = true;
        if (!argv.localPath) {
          argv.localPath = '.';
        }
      }
